    ...
    <div className="min-h-screen bg-background">
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-card/95 backdrop-blur md:hidden">
        <div className="mx-auto flex max-w-6xl">
          <BottomLink to="/" label="儀表板" icon={<LayoutDashboard className="h-5 w-5" />} active={pathname === "/"} />
          <BottomLink to="/projects" label="案件" icon={<FolderKanban className="h-5 w-5" />} active={isActive("/projects")} />
...
function BottomLink({
  to, label, icon, active, primary,
}: { to: string; label: string; icon: ReactNode; active: boolean; primary?: boolean }) {
  return (
    <Link
      to={to}
