...
import { addWatermark, nowStamp } from "@/lib/watermark";
import { savePhoto, uid, type PhotoCategory, type Project } from "@/lib/storage";
  ...
  async function handleFiles(files: FileList | null) {
    ...
    try {
      ...
      for (let i = 0; i < arr.length; i++) {
        ...
        const dataUrl = await addWatermark(file, {
  ...
  return (
    <div className="card-surface p-4">
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="text-[11px] font-medium text-accent-foreground">✦ 自動加浮水印</span>
      </div>
      ...
      {progress && (
        <p className="mt-2 text-center text-xs text-muted-foreground">
          已處理 {progress.done} / {progress.total}
        </p>
      )}
...
export function PhotoLightbox({
  ...
  photos: { dataUrl: string; takenAt: string; note?: string }[];
  index: number;
  onClose: () => void;
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-black/90 backdrop-blur" onClick={onClose}>
      <button
        onClick={onClose}
        className="absolute right-3 top-3 z-10 grid h-10 w-10 place-items-center rounded-full bg-white/10 text-white hover:bg-white/20"
      >
        <X className="h-5 w-5" />
      </button>
      <div className="flex flex-1 items-center justify-center p-4" onClick={(e) => e.stopPropagation()}>
        <img src={p.dataUrl} alt="" className="max-h-full max-w-full rounded-lg object-contain" />
      </div>
      <div className="flex items-center justify-between gap-3 px-4 py-3 text-white" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="text-center text-xs">
          <div className="font-semibold">{p.takenAt}</div>
          {p.note && <div className="opacity-70">{p.note}</div>}
          <div className="mt-0.5 opacity-50">{index + 1} / {photos.length}</div>
        </div>
        <button
