...
import { DEFAULT_QUOTE_TERMS, formatPaymentScheduleText, isAutoGeneratedPaymentSchedule, resolvePaymentScheduleText, resolveQuoteTerms } from "@/lib/quote-document.utils";
...
function QuoteEditorPage() {
  ...
  useEffect(() => {
    if (data) {
      ...
      const hasCustomPayment = Boolean(
        data.payment_schedule?.trim() && !isAutoGeneratedPaymentSchedule(data.payment_schedule),
      );
      setForm({
        ...data,
        quote_lines: undefined,
        terms: data.terms?.trim() || DEFAULT_QUOTE_TERMS,
        payment_schedule: resolvePaymentScheduleText(loadedTotals.total, data.payment_schedule),
      });
  ...
  const quotePreview = form
    ? {
        ...
        payment_schedule: resolvePaymentScheduleText(
          totals.total,
          paymentTouched ? form.payment_schedule : null,
        ),
      }
    : null;

  const paymentValue = resolvePaymentScheduleText(
    totals.total,
    paymentTouched ? form?.payment_schedule : null,
  );
  ...
  const saveMut = useMutation({
    mutationFn: async ({ silent: _silent = false }: { silent?: boolean } = {}) => {
      ...
      const pay = resolvePaymentScheduleText(
        s.totals.total,
        s.paymentTouched ? s.form.payment_schedule : null,
      );
      await apiSaveQuote(
        () =>
          saveFn({
