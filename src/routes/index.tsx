...
import { listProjects, listLogs } from "@/lib/storage";
import { useStoreVersion } from "@/hooks/use-storage";

export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "儀表板 — 水電施工紀錄 Pro" },
      { name: "description", content: "管理水電工程案件、施工日誌與現場照片，自動加上案件浮水印。" },
...
function Dashboard() {
  ...
  const todayProjectIds = new Set(
    projects.flatMap((p) => listLogs(p.id).filter((l) => l.date === today).map(() => p.id)),
  );
  const todayProjects = projects.filter((p) => todayProjectIds.has(p.id));
...
function Stat({
  ...
  tone: "primary" | "done" | "accent" | "muted";
}) {
  const toneCls = {
  ...
  return (
    <div className="card-surface p-4">
      <div className={`mb-3 inline-grid h-9 w-9 place-items-center rounded-lg ${toneCls}`}>
        {icon}
      </div>
  ...
  return (
    <div className="card-surface flex flex-col items-center gap-3 px-6 py-12 text-center">
      ...
      <p className="max-w-sm text-sm text-muted-foreground">
        建立第一個案件，開始記錄施工日誌、上傳工地照片。
      </p>
      <Link
        to="/projects/new"
        className="btn-touch mt-1 inline-flex items-center gap-1.5 rounded-lg bg-primary px-4 text-sm font-semibold text-primary-foreground shadow-[var(--shadow-elevated)] hover:brightness-110"
      >
