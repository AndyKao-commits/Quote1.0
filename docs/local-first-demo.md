...
> **正式站請勿設 `VITE_LOCAL_FIRST=true`。**  
> 正式版為雲端登入 + PWA；訊號差時使用本機快取（`src/lib/offline-cache`），不是整站切換成此模式。
...
正式站維持 **雲端版**（Supabase 登入／註冊、報價同步雲端）。  
離線能力由 **PWA + 本機快取** 提供：有網路時自動快取，訊號差時可瀏覽已開過的報價。

**不要**在 Vercel 設 `VITE_LOCAL_FIRST=true`（那是開發用整站離線模式）。

### 開發用整站離線（僅本機）

若要在本機測試「資料完全不進雲端」的實驗模式：
